...
> executing-plans for execution. The user subsequently authorized implementation,
> normal reviewed publication/merge and one Staging promotion until completion.
...
- [x] Confirm approved coding authorization, focused branch, clean baseline and
...
- [x] Give a fresh test author the frozen spec and repository test patterns.
...
- [x] Use deterministic fake interfaces. For older-ancestor acceptance, the fake
...
- [x] Run `uv --directory services/api run --locked --no-sync pytest
...
- [x] Parent approves test adequacy and scope, including each mutant and AC item,
  before handing approved tests/contracts to a separate implementer.

### 2. Implement the single supported operator

- [x] Implement safe argument validation and fixed target/evidence path.
...
- [x] Implement commit resolution, remote immutable-main capture and comparison,
...
- [x] Verify evidence writability and canonical Staging source, disabled triggers,
...
- [x] Implement exact-D target/status/event observation, cursor pagination,
...
- [x] Select actual RUNNING instance belonging to D, explicitly pass instance ID
...
- [x] Execute existing canonical HTTP smoke with captured output. Record only
...
- [x] Handle mutation-response loss and record-write failure explicitly, without
...
- [x] Register the script in existing formatting/lint/Pyright/Semgrep and backend
...
- [x] Update the Staging runbook to link the frozen contract and single supported
...
- [x] Run focused operator tests, then affected CI/command-contract/Semgrep tests.
...
- [x] Dispatch a fresh Compact reviewer with spec, tests, final diff and check
