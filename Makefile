...
api-format-check:
	...
	$(API_UV) run --locked --no-sync ruff format --check . $(SMOKE_SCRIPT) $(AUTH_SMOKE_SCRIPT) $(REPLACEMENT_AUTH_SMOKE_SCRIPT) $(STAGING_AUTH_SMOKE_SCRIPT) $(ENVIRONMENT_FINGERPRINT_SCRIPT) $(MEDIA_STORAGE_SMOKE_SCRIPT) $(DEPLOYMENT_IDENTITY_SCRIPT) $(STAGING_PROMOTION_SCRIPT) $(STAGING_PREFLIGHT_SCRIPT) $(STAGING_OPERATOR_INSPECTOR_SCRIPT) $(STAGING_OPERATOR_MATRIX_SCRIPT) $(STAGING_OPERATOR_MATRIX_SSH_SCRIPT) $(STAGING_EMERGENCY_OPERATOR_SSH_SCRIPT) $(STAGING_EMERGENCY_DECOMMISSION_SSH_SCRIPT) $(DEVELOPMENT_DELIVERY_EVENT_SCRIPT) $(STAGING_RESET_SCRIPT) $(STAGING_RESET_SSH_SCRIPT) $(STAGING_RESTORE_SCRIPT) $(STAGING_RESTORE_INTEGRITY_SCRIPT) $(CLERK_DEVELOPMENT_SESSION_SCRIPT) $(CI_RELEVANCE_SCRIPT) $(SEMGREP_VALIDATOR)
