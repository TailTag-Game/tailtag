"""Launch one exact-instance interactive #243 managed-password rotation."""
...
_SOURCES: Final = {
    ...
    "rotation_command": _ROOT
    / "services"
    / "api"
    / "accounts"
    / "management"
    / "commands"
    / "rotate_staging_managed_password.py",
}
...
def _valid(request):
    ...
    if not isinstance(sources, dict) or set(sources) != {"inspector", "recovery_command", "rotation_command"}: return False
    ...
    return True
def _fail():
    print("FAIL_MANAGED_PASSWORD_ROTATION_UNCERTAIN", file=sys.stderr)
    raise SystemExit(1)
try:
    ...
    with contextlib.redirect_stdout(captured), contextlib.redirect_stderr(captured):
        ...
        rotation_module = types.ModuleType("accounts.management.commands.rotate_staging_managed_password")
        rotation_module.__file__ = "/app/rotate_staging_managed_password.py"
        rotation_module.__package__ = "accounts.management.commands"
        sys.modules[rotation_module.__name__] = rotation_module
        exec(compile(sources["rotation_command"]["source"], rotation_module.__file__, "exec"), rotation_module.__dict__)
    if captured.getvalue(): raise ValueError
except BaseException:
    _fail()
try:
    rotation_module.Command().execute(expected_identity=identity, force_color=False, no_color=False, skip_checks=True)
except BaseException:
    _fail()
print("TAILTAG_MANAGED_PASSWORD_ROTATION_COMPLETED")
"""
...
def run() -> dict[str, object]:
    """Guard one password rotation; transport success still needs postcondition proof."""
    started = _now()
